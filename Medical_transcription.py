...
class MedicalTranscriptionSystem:
    ...
    def process_consultation(self, transcript: str
                             ) -> Tuple[PatientInfo,
                                        List[str]]:
    ...
    def generate_missing_data_prompt(self, missing_fields):
        # Construction du contexte selon le domaine
        current_context = {
                "intro": "En tant qu'assistant médical, pour mieux vous aider",
                "importance": "pour assurer un suivi médical approprié",
                "tone": "professionnel et rassurant"
            }

        # Construction du prompt
        prompt = f"""
En tant qu'assistant IA, rédigez un message poli demandant des informations complémentaires.

Contexte:
- {current_context['intro']}
- Ces informations sont nécessaires {current_context['importance']}
- Le ton doit être {current_context['tone']}

Informations manquantes à demander:
{', '.join(missing_fields)}

Consignes pour la réponse:
1. Commencez par une formule polie
2. Expliquez brièvement pourquoi ces informations sont nécessaires
3. Listez clairement les informations demandées
4. Terminez par une formule de remerciement
5. Gardez un ton {current_context['tone']}
6. Évitez les formulations trop longues ou complexes

Générez un message naturel et cohérent qui demande ces informations.

"""

        answer = self.patient_extractor._generate_text(prompt)
        lines = answer.split('\n')
        target_index = lines.index("Générez un message naturel et cohérent qui demande ces informations.")
